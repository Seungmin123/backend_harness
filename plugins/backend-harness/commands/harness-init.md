...
2. `test -x ./mvnw || test -x ./gradlew` — 빌드 도구 wrapper가 없으면 경고한다(post-edit
   자동 테스트·pre-commit Checkstyle이 동작하지 않음). 중단하지는 않는다.
   감지된 wrapper에 따라 `BUILD_TOOL` 값(maven|gradle)을 기록한다.
...
스택/인프라/의존성/테스트 스택은 프로젝트의 빌드 파일(`pom.xml` 또는 `build.gradle`(.kts))을 읽어 스스로 파악한다
(Java 버전, Spring Boot 버전, 주요 스타터, 테스트 라이브러리). 파악 불가 항목만 추가로 묻는다.
`JAVA_VERSION`/`SPRING_BOOT_VERSION`은 빌드 파일에서 읽은 값을 그대로 기록한다(추측 금지).
빌드 파일이 아직 없는 신규 프로젝트면 이 두 값도 사용자에게 묻는다.
...
6. **.gitignore 등록**: `.gitignore`에 `chain-report.json` 항목이 없으면 추가한다
   (체인 스킬이 남기는 로컬 작업 상태 파일 — 커밋 대상이 아니다).
...
- 남은 수동 확인 1가지: 기존 코드가 있는 프로젝트라면 코드 스타일 검사(`./mvnw checkstyle:check`
  또는 `./gradlew checkstyleMain checkstyleTest`)를 한 번 돌려
