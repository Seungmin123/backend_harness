...
SUMMARY: 구현 내용 요약 (다음 에이전트에 전달할 컨텍스트).
         **Phase 1 #7에서 결정한 인증 방식(JWT / 세션)과 신규 엔드포인트 접근 권한을 반드시 포함**한다
...
0. **시스템 감사**: Plan 작성 전에 `git status`로 작업 트리 상태를 확인하고 결과를 Plan에
   표기한다. 미커밋 변경이나 충돌이 있으면 그 사실을 알리고 처리 방향(스태시/별도 커밋/그대로 진행)을
   확인받는다 — 체인의 RED/GREEN 커밋에 이번 요청과 무관한 변경이 섞이는 것을 방지한다.
1. **가정 및 대안** (`.claude/rules/engineering-guidelines.md` ENG-01): 요청에 불확실하거나 여러
   해석이 가능한 부분이 있으면 가정을 명시하고, 더 단순한 대안이 있다고 판단되면 이유와 함께
   제안한다. 판단이 서지 않으면 진행하지 말고 되묻는다.
   **각 가정에 위험도를 붙인다** — `H`(틀리면 설계 재작업), `M`(틀리면 일부 구현 재작업),
   `L`(틀려도 국소 수정). `H` 가정은 추정으로 진행하지 않고 반드시 사용자에게 확인받는다.
2. **엔드포인트 목록**: 메서드, URL, 역할
3. **요청/응답 스키마**: 주요 필드와 타입
4. **레이어 구조 및 모듈 배치**: 새로 생성할 클래스 목록과 각 클래스의 소속 모듈.
   모듈러 모놀리스 프로젝트면(`docs/ARCHITECTURE.md`에 "모듈/서비스 지도"가 있으면) 새 클래스가
   속할 모듈을 명시하고, 모듈 간 의존이 생기는 경우 그 방향이 지도에 허용된 방향인지·`LAYER-02`
   (모듈 경계)에 어긋나지 않는지 Plan에서 검증한다. 지도에 없는 의존이 필요하면 구현 전에
   사용자에게 지도 갱신을 제안한다.
5. **트랜잭션 경계**: 어느 Service 메서드에 `@Transactional` 적용할지
6. **예외 시나리오**: 발생 가능한 비즈니스 예외와 HTTP 상태 코드 매핑
7. **인증 방식 및 접근 권한**: 프로젝트 인증 방식(**JWT / 세션**)과 신규 엔드포인트의 접근 권한
...
8. **검증 기준** (`.claude/rules/engineering-guidelines.md` 4번): 각 구현 단계가 끝났다는 것을
   무엇으로 확인할지 명시한다.
   ```
   ...
   ```
9. **태스크 크기**: 엔드포인트 3개 초과 또는 신규 클래스 10개 초과면 분할안을 함께 제시한다
