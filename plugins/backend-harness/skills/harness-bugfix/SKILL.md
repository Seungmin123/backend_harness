...
다른 체인(`harness-api-build`)은 구현이 테스트보다 먼저다. 하지만 버그 수정은 "무엇이 잘못됐는지"를
코드로 고정(=실패 테스트)하지 않고 고치면, 실제로 그 버그를 고쳤는지 확인할 방법이 없다
(`.claude/rules/engineering-guidelines.md` 4번 "목표 중심으로 실행하라" 참조).
