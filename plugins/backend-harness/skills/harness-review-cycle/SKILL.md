...
> **이 스킬의 Write/Edit 권한 용도 제한**: `allowed-tools`의 Write/Edit은
> **`chain-report.json` 갱신과 `.gitignore` 등록 전용**이다. 이 스킬은 조율자다 — 소스 코드
> 수정은 항상 fix 담당 에이전트를 호출해서 수행하며, 스킬 컨텍스트에서 `.java`/설정 파일을
> 직접 고치지 않는다 (고치면 code-reviewer의 독립 검토 대상에서 벗어난 수정이 생긴다).

## 기본 규칙

- **최대 반복 횟수**: 3회. 초과 시 사용자에게 에스컬레이션하고 자동 진행을 중단한다.
- **PASS_WITH_WARNINGS**: 재검토를 트리거하지 않는다. 경고 항목은 SUMMARY에 기록 후 종료.
- **반복 카운터 (이 스킬이 단일 원천)**: 회차는 이 스킬이 관리하며 `REVIEW_CYCLE: N/3`으로
  `code-reviewer`에 전달한다. `code-reviewer`는 전달받은 값을 `[REVIEW CYCLE N/3]` 형식으로
  첫 줄에 표기만 한다 — 회차를 스스로 세거나 갱신하지 않는다.
...
이슈의 rule ID 프리픽스가 1차 기준이고, rule ID가 `GEN`이면 유형 설명으로 판단한다.

| FAIL 이슈 유형 (rule ID 프리픽스) | 수정 담당 에이전트 |
|---|---|
| 요구사항 누락, 레이어 구현 오류, OpenAPI 누락, API 컨벤션 위반 (`API-xx`) | `api-developer` |
| `@Valid` 누락, 보안 정책 위반, 시크릿 하드코딩 (`SEC-xx`) | `security-checker` |
| 테스트 누락, 커버리지 미달, TDD RED 증거 부재 (`TEST-xx`) | `qa-engineer` |
| 레이어 경계 위반, 설계 원칙 위반 (`LAYER-xx`, `ENG-xx`) | `code-quality` |
| 타임아웃·Retry·Circuit Breaker·Graceful Shutdown 누락, 트랜잭션 내 외부 호출 (`RES-xx`) | `ops-checker` |
| 로깅 누락, Trace ID 전파 누락, 핵심 메트릭/헬스체크 누락 (`OBS-xx`) | `ops-checker` |
| N+1, 인덱스 미사용, 페이지네이션 부재 등 성능 결함 (`PERF-xx`) | `perf-analyzer` |
...
수정 에이전트의 작업이 끝나면 `code-reviewer`를 재호출하기 **전에** 전체 테스트
(`./mvnw test` 또는 `./gradlew test`)를 실행해 green임을 확인한다. red면 재검토를 진행하지
않고, 실패 테스트를 `FOCUS`에 추가해 해당 fix 담당을 다시 호출한다 (이 재시도도 같은 회차에
포함 — 회차 카운터를 올리지 않는다). `code-reviewer`는 기계 검증이 통과한 상태를 전제로
호출된다.
...
- `FOCUS`: 직전 code-reviewer가 지적한 FAIL 항목 목록을 명시.
  **원천은 `chain-report.json`의 `review_cycle.issues`다** — in-context 텍스트에서 재구성하지
  말고 파일에 기록된 구조화 이슈에서 가져온다 (세션이 끊겨도 유실되지 않게 하기 위함).
...
```json
{
  ...
  "verdict": "FAIL",
  "issues": [
    { "severity": "FAIL", "type": "요구사항 누락", "file": "UserService.java",
      "desc": "existsByEmail 미구현", "owner": "api-developer" }
  ],
  "unresolved": ["..."]
}
```

- `issues`: 직전 `code-reviewer`의 FAIL 항목을 **구조화해 기록**한다 — 다음 회차 수정
  에이전트·재검토의 `FOCUS`는 이 배열에서 구성한다 (in-context 텍스트 의존 금지).
- `unresolved`: 사이클 종료(에스컬레이션 포함) 시점에 남은 이슈의 요약.
- 3회 초과로 에스컬레이션되면 `"verdict": "ESCALATED"`로 남겨, 세션이 끊긴 뒤에도 `/review`로
  재진입할 때 몇 회차까지 진행됐는지 알 수 있게 한다.
- `chain-report.json`은 커밋하지 않는다 (`harness-api-build`의 산출물 규칙과 동일 —
  `/harness-init`이 `.gitignore`에 등록).
