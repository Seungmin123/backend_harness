...
4. `security-checker` 호출 → `VULNERABILITIES`, `SNIPPETS`, `CVE_UPGRADES` 수신
5. `ops-checker` 호출 → `ISSUES`, `SNIPPETS` 수신
6. **기계 검증 게이트 (code-reviewer 선행 조건)**: 전체 테스트(`./mvnw test` 또는
...
7. `code-reviewer` 호출. `PRIOR_AGENTS: security-checker, ops-checker` 전달, `CONTEXT`에는
   **최초 사용자 요청 원문**(1~5단계 SUMMARY가 아님)을 전달한다.
   code-reviewer는 Step 4에서 `chain-report.json`의 `tdd.red_confirmed`를 검증한다.
8. `VERDICT: FAIL` → `harness-review-cycle` 스킬 문서로 위임한다. 사이클 진입 방식은
