...
| 에이전트 | 책임 | tools | model |
|---|---|---|---|
| api-developer | REST API 설계 및 구현 | Read, Write, Edit, Grep, Glob, Bash(mvnw compile/test) | 세션 상속 |
| qa-engineer | 테스트 생성 및 커버리지 분석 | Read, Write, Edit, Grep, Glob, Bash(mvnw test) | sonnet |
| code-quality | 설계 원칙 및 코드 품질 | Read, Grep, Glob, Edit, Bash(mvnw checkstyle:check) | sonnet |
| perf-analyzer | 성능 이슈 탐지 | Read, Grep, Glob, Edit, Bash(mvnw test) | sonnet |
| security-checker | 보안 취약점 탐지 | Read, Grep, Glob, Edit, Bash(mvnw dependency-check:check) | sonnet |
| ops-checker | 복원력(타임아웃·Circuit Breaker·Graceful Shutdown) 및 관찰성(로깅·메트릭·헬스체크) | Read, Grep, Glob, Edit, Bash(mvnw test) | sonnet |
